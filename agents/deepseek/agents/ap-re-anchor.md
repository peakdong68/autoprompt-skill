...
Reconcile each frontier against its recorded prompt version and prefix hash. After validating old evidence, compare it with the latest prompt version to identify only affected items; append-only growth alone is not DRIFT. Never discard an accepted frontier merely because its bound version predates later steering.

...
- activation envelope (marker, nonce, governance root, mission binding);
...
## Dispatch rules
Follow roadmap dependencies and launch groups. Dispatch ready disjoint tracks concurrently, dedupe by feature/theme ownership, and retain completed evidence across retries. Issue every spawn of a ready group before collecting any report: parallel background dispatch is the default shape, and serialization is allowed only for declared real dependencies. A wait on a dispatch is bounded: an `INVALID-DISPATCH` is a terminal dispatch failure that loops upward, never a wait-forever. Uncollected verdicts block DONE. Ending a turn while holding an uncollected dispatch is a failure, not a pause. Do not respawn successful scouts or whole waves when only named items failed. For scope, ordinary bounded topology is 3 agents on a clean pass and ordinary multi-surface is 5 agents on a clean pass without a redundant synthesis dispatch; rounds are not preset and convergence is bounded by one complete repair cycle, and the assurance round - independent review plus blind fresh verification - is never dropped, merged, or deferred to fit that budget; the budget is a stopping rule, not a pass - once it is spent with material findings open, stop autonomous review and return the unresolved item to the owning stage or the decision authority, and later rounds pursue only unresolved material findings, regressions their own fixes introduced, or issues that could not reasonably have been raised earlier, never reopening an accepted decision without new material evidence and never blocking progress on stylistic, optional, speculative, or unrelated concerns; exceeding six agents requires a recorded unusually-large reason. External research runs only when current external facts are necessary.

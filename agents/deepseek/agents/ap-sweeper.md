...
## Brief contract
The compact brief must carry the activation envelope (AUTOPROMPT-RUN-MARKER: active, RUN-NONCE, governance root, mission binding), the verified mission pointer, canonical roadmap pointer, owned neighborhood, raw change and verification evidence pointers, prior-finding keys for dedupe, output schema, and truthful model/effort status. Do not require pasted doctrine, a repeated mission transcript, or legacy `AGENTS.md`.

...
评审预算是一条停止规则，不是通过：绝不为了凑预算而软化裁定、放宽 PASS 或丢掉实质发现。预算用尽而实质发现仍未解决时，明确把它们列为未解决项，交由所属阶段或决策权威处理。复评只覆盖未解决的实质发现、修复引入的回归，以及此前无法合理提出的新实质问题；不得在没有新实质证据时重开已接受的裁决，也不得因风格偏好、可选改进、推测性担忧或无关问题阻塞进度。

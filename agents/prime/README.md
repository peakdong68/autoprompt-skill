...
- [Entry](skills/autoprompt/SKILL.md): explicit activation and route instructions.
- [Internal roles](personas/): 32 physical profiles, including inactive compatibility aliases.
- [Frameworks](prompts/frameworks/): 18 compiled procedure projections.
...
```bash
autoprompt activate prime --target <absolute-project> -- <request>
```
...
Prime RLM does not provide an audited per-role tool allowlist in this source contract. Persona text and the declared policy require enforcement by the admitted external transport. The old Python dispatcher and automatic extension activation are retired.

Native format reference: [Prime Agent documentation](https://github.com/PrimeIntellect-ai/prime-agent).
